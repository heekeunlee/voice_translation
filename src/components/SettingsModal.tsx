...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="relative w-full max-w-xl rounded-3xl bg-slate-900 border border-slate-800 p-6 sm:p-8 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Gemini 2.0 Flash 초저지연 스트리밍 및 음성 옵션을 설정하세요.</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-indigo-300">Gemini 2.0 Flash</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-semibold">초저지연 추천</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">Google 차세대 초고속 멀티모달 번역</p>
              </button>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-indigo-300">스마트 로컬 엔진</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-semibold">무료 / No Key</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">API 키 없이 즉시 브라우저에서 사용 가능</p>
              </button>
            </div>
          </div>
          ...
          {formData.engine.startsWith('gemini') && (
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-slate-200 placeholder-slate-500 outline-none focus:border-indigo-500 transition font-mono"
              />
